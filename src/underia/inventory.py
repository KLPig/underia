...
class Inventory:
    ...
    class Item:
        ...
        def handle_data(desc):
            ...
            elif desc.endswith('sprint'):
                cdata['splint'] = int(desc.removesuffix('sprint'))
            elif desc.endswith('%sprintcd'):
                cdata['splint_cd'] = int(desc.removesuffix('%sprintcd'))
        ...
        def __init__(self, name, description, identifier: str, rarity: int = 0, tags: list[Tag] = [],
            ...
            self.accessory_data = {}
            self.conditioned_accessory_data: dict[tuple[object, str, float], bool] = {}
        ...
        def get_full_desc(self):
            ...
            if TAGS['weapon'] in self.tags:
                ...
                if weapon.damages:
                    d = f"{min(1.0, weapon.critical + game.get_game().player.strike) * 100:.0f}% critical\n" + d
                for dmg, val in weapon.damages.items():
                    ...
                    d = f"{int(_dmg) if type(_dmg) is float else _dmg} {damages.NAMES[dmg]} {dt}\n" + d
            ...
            if TAGS['weapon'] in self.tags and constants.DIFFICULTY > 1:
                ...
                if issubclass(type(weapon), weapons.SweepWeapon):
                    d += 'col7fff7fMastery: -50% immune time\n'
...
items_dict: dict[str, Inventory.Item] = {
    ...
    'floatstone': Inventory.Item('Floatstone', '', 'floatstone', 2, [TAGS['item']]),
    'obsidian_ingot': Inventory.Item('Obsidian Ingot', 'Unbelievable toughness.', 'obsidian_ingot', 2, [TAGS['item']]),
    'coral_reef': Inventory.Item('Coral Reef', '', 'coral_reef', 3, [TAGS['item']]),
    'flufffur': Inventory.Item('Flufffur', '', 'flufffur', 2, [TAGS['item']]),
    ...
    'bloodstone': Inventory.Item('Bloodstone', '', 'bloodstone', 2, [TAGS['item']]),
    ...
    'sea_pearl': Inventory.Item('Sea Pearl', '', 'sea_pearl', 2, [TAGS['item']]),
    ...
    'thermal_ingot': Inventory.Item('Thermal Ingot', '', 'thermal_ingot', 6, [TAGS['item']]),
    'biome_alloy': Inventory.Item('Biome Alloy', 'Power of life.', 'biome_alloy', 7, [TAGS['item']]),
    'sun_shadow_ingot': Inventory.Item('Sunshadow Ingot', '', 'sun_shadow_ingot', 7, [TAGS['item']]),
    ...
    'mystery_core': Inventory.Item('Mystery Core', 'Knowledge\n+1 max talent\n+5 talent', 'mystery_core', 7,
                                   [TAGS['item']]),
    'spirit_essence': Inventory.Item('Spirit Essence', 'br_chaosWith spirit as combinator.', 'spirit_essence', 7, [TAGS['item']]),
    ...
    'incremental_spirit_essence': Inventory.Item('Incremental Spirit Essence', 'br_chaosImmersed in increasing power.',
                                                   'incremental_spirit_essence', 7, [TAGS['item']]),
    ...
    'seed_of_origin': Inventory.Item('Seed of Origin', 'The memory from the old days starts to grow out new leaves.', 'seed_of_origin', 9, [TAGS['item']]),
    ...
    'soul_of_determination': Inventory.Item('Soul of Determination', 'Willpower of non-regretted chooses.\n'
                                                                     '20% chance to reach the limit of human souls.\nrainbowThe law of NIRVANA is staying inside.'
    ...
    'origin_feather': Inventory.Item('Origin Feather', 'Not afraid of time.', 'origin_feather', 10, [TAGS['item']]),
    'scorch_core': Inventory.Item('Scorch Core', '', 'scorch_core', 9, [TAGS['item']]),
    'curse_core': Inventory.Item('Curse Core', '', 'curse_core', 9, [TAGS['item']]),
    ...
    'ascendant_spirit_essence': Inventory.Item('Ascendant Spirit Essence', 'br_chaosUntil exceeding the limit of humans.', 'ascendant_spirit_essence', 11, [TAGS['item']]),
    'the_final_soul': Inventory.Item('The Final Soul', 'Your finale of THIS journey.', 'the_final_soul', 12, [TAGS['item']]),
    ...
    'magic_defense': Inventory.Item('Magic Defense', '', 'magic_defense', 2,
                                     [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'sky_edge': Inventory.Item('Sky Edge', '', 'sky_edge', 2, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'doctor_expeller': Inventory.Item('Doctor Expeller', 'Daily apple, daily non-doctor.', 'doctor_expeller', 3,
                                       [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'angels_tears': Inventory.Item('Angels Tears', '', 'angels_tears', 3,
                                    [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'hellseal': Inventory.Item('Hellseal', 'Fire of fear silently burns.', 'hellseal', 5, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'excalibur': Inventory.Item('Excalibur', 'colffff7fThe legendary sword of hallow.', 'excalibur', 6,
                                [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'burning_tear': Inventory.Item('Burning Tears', '', 'burning_tear', 6,
                                   [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'glacier': Inventory.Item('Glacier', 'Chance to freeze enemies.', 'glacier', 6, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'hidden_heat': Inventory.Item('Hidden Heat', '', 'hidden_heat', 6,
                                   [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'remote_sword': Inventory.Item('Remote Sword', '', 'remote_sword', 6,
                                    [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'wave': Inventory.Item('Wave', '', 'wave', 6,
                           [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'fiery_iceberg': Inventory.Item('Fiery Iceberg', 'Chance to freeze or ignite enemies.', 'fiery_iceberg', 7,
                                    [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'ice_crystal_edge': Inventory.Item('Ice Crystal Edge', 'Chance to freeze enemies for 2s.', 'ice_crystal_edge', 7,
                                       [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'black_hole_sword': Inventory.Item('The Black Hole Sword', 'Attracts enemies.', 'black_hole_sword', 6,
                                       [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'incremental_broadsword': Inventory.Item('Incremental Broadsword', '', 'incremental_broadsword', 6,
                                              [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'diamond': Inventory.Item('Diamond', '', 'diamond', 8, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'seedler': Inventory.Item('Seedler', '', 'seedler', 9, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'ark_of_elements': Inventory.Item('Ark of Elements', '', 'ark_of_elements', 9, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'star_defense': Inventory.Item('Star\'s Defense', '', 'star_defense', 8, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'viper_spiker': Inventory.Item('Viper Spiker', '', 'viper_spiker', 1, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'obsidian_polearm': Inventory.Item('Obsidian Polearm', '', 'obsidian_polearm', 1, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'poseidon': Inventory.Item('Poseidon', 'Sprints toward enemy.(max. 2)\nEffects increases in water bodies or shallow abyss.', 'poseidon',
                               4, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'vortex_spike': Inventory.Item('Vortex Spike', '', 'vortex_spike', 4, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'nights_pike': Inventory.Item('Night\'s Pike', 'colff7fffThe sunset has gone, it\'s now night...', 'nights_pike', 4,
                                  [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    #'burndent': Inventory.Item('Burndent', 'When hit, burns target.', 'burndent', 4, [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'thermal_pike': Inventory.Item('Thermal Pike', '', 'thermal_pike', 6,
                                    [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'star_wrath': Inventory.Item('Star Wrath', 'The wingdings from the starry sky.', 'star_wrath', 9,
                                  [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    'eternal_sunfire': Inventory.Item('Eternal Sunfire', '', 'eternal_sunfire', 9,
                                       [TAGS['item'], TAGS['weapon'], TAGS['melee_weapon']]),
    ...
    'star_of_devotion': Inventory.Item('Star of Devotion', 'rainbowUnder the star, is all in faith.', 'star_of_devotion', 12,
                                        [TAGS['item'], TAGS['weapon'], TAGS['ce_item'], TAGS['melee_weapon']]),
    'mind': Inventory.Item('Mind', '', 'mind', 12,
                            [TAGS['item'], TAGS['weapon'], TAGS['ce_item'], TAGS['melee_weapon']]),
    'adherent': Inventory.Item('Adherent', '', 'adherent', 12,
                                [TAGS['item'], TAGS['weapon'], TAGS['ce_item'], TAGS['melee_weapon']]),
    'worlds_lord': Inventory.Item('World\'s Lord', '', 'worlds_lord', 12,
                                  [TAGS['item'], TAGS['weapon'], TAGS['ce_item'], TAGS['melee_weapon']]),
    ...
    'vipers_breath': Inventory.Item('Viper\'s Breath', '', 'vipers_breath', 1, [TAGS['item'], TAGS['weapon'], TAGS['bow']]),
    ...
    'leading_ray': Inventory.Item('Leading Ray', '', 'leading_ray', 3, [TAGS['item'], TAGS['weapon'], TAGS['bow']]),
    ...
    'club': Inventory.Item('Club', '', 'club', 8, [TAGS['item'], TAGS['weapon'], TAGS['bow']]),
    ...
    'the_desert_eagle': Inventory.Item('The Desert Eagle', '', 'the_desert_eagle', 3, [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    ...
    'minishark': Inventory.Item('Minishark', '', 'minishark', 2,
                                 [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    'burst': Inventory.Item('Burst', 'Shoots 3 bullets at once.', 'burst', 3, [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    ...
    'dark_exploder': Inventory.Item('Dark Exploder', '', 'dark_exploder', 5,
                                     [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    'fire_shine': Inventory.Item('Fire Shine', '', 'fire_shine', 5,
                                     [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    ...
    'chaos_assaulter': Inventory.Item('Chaos Assaulter', '', 'chaos_assaulter', 8,
                                         [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    'venus_magnum': Inventory.Item('Venus Magnum', '', 'venus_magnum', 9,
                                         [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    'void': Inventory.Item('Void', 'Nothing...', 'void', 11, [TAGS['item'], TAGS['weapon'], TAGS['gun']]),
    'climax': Inventory.Item('Climax', 'Reaches the highest, drop till the lowest.', 'climax', 11,
    ...
    'magic_arrow': Inventory.Item('Magic Arrow', '', 'magic_arrow', 1,
                                  [TAGS['item'], TAGS['ammo'], TAGS['ammo_arrow']]),
    'snowball': Inventory.Item('Snowball', '', 'snowball',
    ...
    'chloro_arrow': Inventory.Item('Chloro Arrow', 'Always face to the target.', 'chloro_arrow', 7,
                                   [TAGS['item'], TAGS['ammo'], TAGS['ammo_arrow']]),
    'chlorommo': Inventory.Item('Chlorommo', '', 'chlorommo', 7, [TAGS['item'], TAGS['ammo'], TAGS['ammo_bullet']]),
    'space_jumper': Inventory.Item('Space Jumper', '', 'space_jumper', 7,
                                    [TAGS['item'], TAGS['ammo'], TAGS['ammo_bullet']]),
    'scorching_arrow': Inventory.Item('Scorching Arrow', '', 'scorching_arrow', 10,
                                       [TAGS['item'], TAGS['ammo'], TAGS['ammo_arrow']]),
    'eterrow': Inventory.Item('Eterrow', '', 'eterrow', 12, [TAGS['item'], TAGS['ammo'], TAGS['ammo_arrow']]),
    ...
    'blood_watcher_wand': Inventory.Item('Blood Watcher Wand', 'Shoots a strong lazer.', 'blood_watcher_wand', 2,
                                   [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_light'],
                                    TAGS['magic_lv_2']]),
    'rock_ball': Inventory.Item('Rock Ball', 'Shoots a rock ball.', 'rock_ball', 2,
    ...
    'talent_book': Inventory.Item('Talent Book', 'Shoots smart bomb trace for enemies.', 'talent_book', 2,
                                  [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_light'],
                                   TAGS['magic_lv_2']]),
    'demon_shard': Inventory.Item('Demon shard', 'Shoots powerfull projectiles', 'demon_shard', 2,
    ...
    'sunfire': Inventory.Item('Sunfire', 'Shoots a seperative fireball.', 'sunfire', 3,
                                     [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_fire'],
                                      TAGS['magic_lv_4']]),
    'cloudcore_staff': Inventory.Item('Cloudcore Staff', 'Shoots a cloudy rune.', 'cloudcore_staff', 3,
                                       [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_water'],
                                        TAGS['magic_lv_3']]),
    ...
    'air_float': Inventory.Item('Air Float', 'Floats in a low distance above the ground.\nIt effects significantly in fallen sea.', 'air_float', 3,
                                 [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_air'],
                                  TAGS['magic_lv_3']]),
    'lily_of_the_valley': Inventory.Item('Lily of the Valley', 'Condenses a powerful beam.', 'lily_of_the_valley', 5,
                                         [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_water'],
                                          TAGS['magic_lv_4']]),
    ...
    'fruit_wand': Inventory.Item('Fruit Wand', 'Makes apple fall from the sky.', 'fruit_wand', 3,
                                  [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_life'],
                                   TAGS['magic_lv_2']]),
    'mana_shine': Inventory.Item('Mana Shine', 'Summon a piercive spike.', 'mana_shine', 3,
                                 [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_energy'],
                                   TAGS['magic_lv_3']]),
    ...
    'dark_spider_lily': Inventory.Item('Dark Spider Lily', 'colff7f7fEndless death, sealed oath, nightmarish revival.',
                                      ...
                                      TAGS['magic_lv_3']]),
    'devils_fire': Inventory.Item('Devil\'s Fire', 'Summon an explosive circle.', 'devils_fire', 4,[TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_fire'],
                                      TAGS['magic_lv_4']]),
    'midnights_wand': Inventory.Item('Midnight\'s Wand', 'colff7fffAll darkness...\nShoots several night energy.', 'midnights_wand', 4,
                                     [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_dark'],
                                      TAGS['magic_lv_4']]),
    'oceanic_current': Inventory.Item('Oceanic Current', 'Summon a slow, repulsive current.', 'oceanic_current', 4,
                                       [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_water'],
                                        TAGS['magic_lv_5']]),
    ...
    'double_watcher_wand': Inventory.Item('Double Watcher Wand', 'Summon two lazer beams in different positions.', 'double_watcher_wand', 6,
                                           [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
                                            TAGS['magic_element_light'], TAGS['magic_lv_4']]),
    'wildsands': Inventory.Item('Wildsands', 'Summon bunches of sands, penetrate 50 defense.', 'wildsands', 6,
                                           [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
                                            TAGS['magic_element_earth'], TAGS['magic_lv_5']]),
    'forbidden_curse__spirit': Inventory.Item('Forbidden Curse: Energy', 'Shoots a magic circle.', 'forbidden_curse__spirit', 7,
                                              [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
                                               TAGS['arcane_weapon'], TAGS['magic_element_energy'],
                                               TAGS['magic_lv_forbidden_curse']]),
    'forbidden_curse__evil': Inventory.Item('Forbidden Curse: Death', 'Shoots a strong evil pulse.', 'forbidden_curse__evil', 7,
                                            [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
    ...
    'forbidden_curse__time': Inventory.Item('Forbidden Curse: Time', 'Adjust the rate of time.',
                                            ...
                                            [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
                                             TAGS['arcane_weapon'], TAGS['magic_element_time'],
                                              TAGS['magic_lv_forbidden_curse']]),
    'fate': Inventory.Item('Fate', 'Summon quick magic arrows', 'fate', 7,
    ...
    'growth': Inventory.Item('Growth', 'Summon a seedling.', 'growth', 7,
                             [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_life'],
                              TAGS['magic_lv_5']]),
    'forbidden_curse__water': Inventory.Item('Forbidden Curse: Water', 'Summon 6 icy monuments.', 'forbidden_curse__water', 7,
                                              [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
                                               TAGS['arcane_weapon'], TAGS['magic_element_water'],
                                               TAGS['magic_lv_forbidden_curse']]),
    'forbidden_curse__fire': Inventory.Item('Forbidden Curse: Fire', 'Summon a static bursting vine.', 'forbidden_curse__fire', 7,
                                              [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
    ...
    'life_wand': Inventory.Item('Life Wand', 'Heal you back to life.', 'life_wand', 7,
                                [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_life'],
                                 TAGS['magic_lv_6']]),
    'dream_violet': Inventory.Item('Dream Violet', 'col00ffffCrystalite blood and magic, the eternal rhapsody.', 'dream_violet', 8,
                                    [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_space'],
                                     TAGS['magic_lv_5']]),
    'forbidden_curse__life': Inventory.Item('Forbidden Curse: Life', 'Summon a strong ball summoning seedlings.', 'forbidden_curse__life', 8,
                                             [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'],
                                              TAGS['arcane_weapon'], TAGS['magic_element_life'],
                                               TAGS['magic_lv_forbidden_curse']]),
    ...
    'chaos_killer': Inventory.Item('Chaos Killer', 'Kills all enemies.', 'chaos_killer', 8,
                                  [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_chaos'],
                                   TAGS['magic_lv_2']]),
    'spade': Inventory.Item('Spade', 'Summon a bunch of chaos spade bullet.', 'spade', 8,
                            [TAGS['item'], TAGS['weapon'], TAGS['magic_weapon'], TAGS['magic_element_chaos'],
                             TAGS['magic_lv_1']]),
    ...
    'vision_resonancer': Inventory.Item('Vision Resonancer', '+14% damage\n+12% melee damage\n+10% critical\n+20% karma reduce\n+36% hallow damage\n+10/sec regeneration',
                                          'vision_resonancer', 5, [TAGS['item'], TAGS['accessory']]),
    ...
    'iron_mask': Inventory.Item('Iron Helmet', '1 armor\n+15% speed', 'iron_mask', 1, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    ...
    'cobalt_helmet': Inventory.Item('Cobalt Helmet', '6 armor\n+3% damage', 'cobalt_helmet', 2, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'cobalt_mask': Inventory.Item('Cobalt Mask', '2 armor\n+7% damage', 'cobalt_mask', 2, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'cobalt_battlegear': Inventory.Item('Cobalt Battlegear', '3 armor\n+3% damage', 'cobalt_battlegear', 2, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    'cobalt_leggings': Inventory.Item('Cobalt Leggings', '2 armor\n+2% damage', 'cobalt_leggings', 2, [TAGS['item'], TAGS['accessory'], TAGS['leg']]),
    ...
    'silver_helmet': Inventory.Item('Silver Helmet', '6 armor\n+2 magic defense', 'silver_helmet', 2, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'silver_mask': Inventory.Item('Silver Mask', '3 armor\n+6 magic defense', 'silver_mask', 2, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'silver_battlegear': Inventory.Item('Silver Battlegear', '4 armor\n+2 magic defense', 'silver_battlegear', 2, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    'silver_leggings': Inventory.Item('Silver Leggings', '3 armor\n+1 magic defense', 'silver_leggings', 2, [TAGS['item'], TAGS['accessory'], TAGS['leg']]),
    ...
    'zirconium_helmet': Inventory.Item('Zirconium Helmet', '4 armor\n-5kg', 'zirconium_helmet', 3, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'zirconium_mask': Inventory.Item('Zirconium Mask', '2 armor\n-15kg', 'zirconium_mask', 3, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'zirconium_battlegear': Inventory.Item('Zirconium Battlegear', '3 armor\n-15kg', 'zirconium_battlegear', 3, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    'zirconium_leggings': Inventory.Item('Zirconium Leggings', '3 armor\n-7kg', 'zirconium_leggings', 3, [TAGS['item'], TAGS['accessory'], TAGS['leg']]),

    'scale_headgear': Inventory.Item('Scale Headgear', '5 armor\n+7% ranged damage', 'scale_headgear', 2, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'scale_chestplate': Inventory.Item('Scale Chestplate', '6 armor\n+9% ranged damage\n+25% critical', 'scale_chestplate', 2, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    'scale_boots': Inventory.Item('Scale Boots', '5 armor\n+5% ranged damage\n+15% speed', 'scale_boots', 2, [TAGS['item'], TAGS['accessory'], TAGS['leg']]),
    ...
    'bloodstone_mask': Inventory.Item('Bloodstone Mask', '2 armor\n-10% mana cost\n+8% magic damage', 'bloodstone_mask',
                                      3, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'bloodstone_cloak': Inventory.Item('Bloodstone Mask', '3 armor\n+80 additional maximum mana\n+12% magic damage', 'bloodstone_mask',
                                      3, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    ...
    'tidal_mask': Inventory.Item('Tidal Mask', '6 armor\n+16% ranged damage', 'tidal_mask', 4, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'tidal_chestplate': Inventory.Item('Tidal Chestplate', '7 armor\n+18% ranged damage\n+35% critical', 'tidal_chestplate', 4, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    'tidal_greaves': Inventory.Item('Tidal Greaves', '6 armor\n+8% ranged damage\n-30% air resistance\n-50% biome friction effect', 'tidal_greaves', 4, [TAGS['item'], TAGS['accessory'], TAGS['leg']]),
    ...
    'everscorch_helmet': Inventory.Item('Everscorch Helmet', '12 armor\n+15% ranged damage', 'everscorch_helmet', 5, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'everscorch_chestplate': Inventory.Item('Everscorch Chestplate', '16 armor\n+20% ranged damage\n+50% critical', 'everscorch_chestplate', 5, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    'everscorch_greaves': Inventory.Item('Everscorch Greaves', '11 armor\n+12% ranged damage', 'everscorch_greaves', 5, [TAGS['item'], TAGS['accessory'], TAGS['leg']]),

    'stardream_hood': Inventory.Item('Stardream Hood', '9 armor\n+9% magic damage\n-25% mana cost', 'stardream_hood', 5, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    ...
    'chaos_vocalist_headgear': Inventory.Item('Chaos Vocalist Headgear', '21 armor\n+40% magic damage', 'chaos_vocalist_headgear', 8, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'chaos_vocalist_crown': Inventory.Item('Chaos Vocalist Crown', '23 armor\n+60% magic damage\n+40% critical\n+200 additional maximum mana', 'chaos_vocalist_crown', 8, [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'chaos_vocalist_shabby_cloak': Inventory.Item('Chaos Vocalist Shabby Cloak', '35 armor\n+300 additional maximum mana\n+10% magic damage', 'chaos_vocalist_shabby_cloak', 9, [TAGS['item'], TAGS['accessory'], TAGS['body']]),
    'chaos_vocalist_traveller_boots': Inventory.Item('Chaos Vocalist Traveller Boots', '17 armor\n+50/sec mana regeneration\n+200 additional maximum mana', 'chaos_vocalist_traveller_boots', 8, [TAGS['item'], TAGS['accessory'], TAGS['leg']]),
    ...
    'plaguebringer_skull': Inventory.Item('Plaguebringer Skull', '27 armor\n+30% magic damage\n+400 additional maximum mana', 'plaguebringer_skull', 10,
                                          [TAGS['item'], TAGS['accessory'], TAGS['head']]),
    'plaguebringer_cloak': Inventory.Item('Plaguebringer Cloak', '35 armor\n+40% magic damage\n+600 additional maximum mana\n-20% mana cost', 'plaguebringer_cloak',  10,
                                           [TAGS['item'], TAGS['accessory'], TAGS['body']]),
...
'natural_necklace': Inventory.Item('Natural Necklace', '+4/sec mana regeneration', 'natural_necklace', 0,
    ...
    'dangerous_necklace': Inventory.Item('Dangerous Necklace', '+9% damage', 'dangerous_necklace', 1,
                                         [TAGS['item'], TAGS['accessory']]),
    'fearful_necklace': Inventory.Item('Fearful Necklace', '+14% critical', 'fearful_necklace', 1,
                                         [TAGS['item'], TAGS['accessory']]),
    ...
    'life_flower': Inventory.Item('Life Flower', '+5/sec regeneration\n-10 touching defense', 'life_flower', 1,
                                   [TAGS['item'], TAGS['accessory']]),
    'war_necklace': Inventory.Item('War Necklace', '+12% damage\n+18% critical', 'war_necklace', 3,
                                   [TAGS['item'], TAGS['accessory']]),
    'magic_anklet': Inventory.Item('Magic Anklet', '15kg\n+4/sec mana regeneration\n+20% speed',
                                   'magic_anklet', 1, [TAGS['item'], TAGS['accessory']]),
    'bloodflower': Inventory.Item('Bloodflower', '+100 additional maximum mana\n-8% mana cost\n+6/sec regeneration',
                                   'bloodflower', 3, [TAGS['item'], TAGS['accessory']]),
    ...
    'bloodstone_amulet': Inventory.Item('Bloodstone Amulet', 'Immune to bleeding\nWhen HP < 25%, +50% damage\nResistant to perilous biomes, e.g. deserts and snows.', 'bloodstone_amulet', 2,
                            [TAGS['item'], TAGS['accessory'], TAGS['light_source'], TAGS['see']]),
    ...
    'hell_giantshield': Inventory.Item('Hell Giantshield', '20kg\n+40 touching defense\n+60 magic defense\n+20 sprint',
                                       'hell_giantshield', 3, [TAGS['item'], TAGS['accessory']]),
    ...
    'titanium_glove': Inventory.Item('Titanium Glove',
                                     ...
                                     'titanium_glove', 5,
                                     [TAGS['item'], TAGS['accessory'], TAGS['night_vision'], TAGS['major_accessory']]),
    'soul_resonancer': Inventory.Item('Soul Resonancer', '+35% critical', 'soul_resonancer',
                                      5, [TAGS['item'], TAGS['accessory']]),
    'angels_amulet': Inventory.Item('Angels Amulet', '+150 additional maximum mana\n+15/sec mana regeneration',
                                     'angels_amulet', 5, [TAGS['item'], TAGS['accessory'], TAGS['major_accessory']]),
    'fires_defense': Inventory.Item('Fire\'s Defense', '25kg\n+45 touching defense\n+45 magic defense\n+80 sprint',
                                    'fires_defense', 5, [TAGS['item'], TAGS['accessory']]),
    'mechanic_boots': Inventory.Item('Mechanic Boots', '10kg\n+120% speed',
    ...
    'cloudy_heavyboots': Inventory.Item('Cloudy Heavyboots', '15kg\n+70% speed\n-20% air resistance',
                                         'cloudy_heavyboots', 7, [TAGS['item'], TAGS['accessory']]),
    ...
    'unregretted_heart': Inventory.Item('Unregretted Heart', 'My choice is right.\n+35% damage\n-18 touching defense\n+12% critical\n+10/sec regeneration',
                                        'unregretted_heart', 5,
                                        [TAGS['item'], TAGS['accessory']]),
    'highspeed_boots': Inventory.Item('Highspeed Boots', '10kg\n+150% speed\n-50% sprint cd',
                                     'highspeed_boots', 5, [TAGS['item'], TAGS['accessory']]),
    ...
    'arrayed_amulet': Inventory.Item('Arrayed Amulet', 'br_chaosImmune to ALL debuffs dealt by biomes(including CHAOS ABYSS).\ncolff0000Speed addons are DISABLED.',
                                     'arrayed_amulet', 11, [TAGS['item'], TAGS['accessory']]),
    ...
    'great_magicians_hat': Inventory.Item('Great Magician\'s Hat', '60kg\n-20% damage\n+72% magical damage\n+100% speed\n+60/sec '
                                                                    'regeneration\n+120 touching defense\n+280 magic defense\n+240/sec mana regeneration\n'
                                                                     '+80% domain size\n+10/sec mentality regeneration', 'great_magicians_hat', 8, [TAGS['item'], TAGS['accessory'],
                                                                                           TAGS['night_vision'], TAGS['major_accessory']]),
    ...
    'starnight_gloves': Inventory.Item('Starnight Gloves', '+50% damage\n-10 attack speed\n+10/sec regeneration\n+80% critical',
    ...
    'magisters_hat': Inventory.Item('Magister\'s Hat', '80kg\n-20% damage\n+77% magical damage\n+80% speed\n+80/sec '
                                                 'regeneration\n+140 touching defense\n+480 magic defense\n+280/sec mana regeneration\n'
                                                  '+220% domain size\n+12/sec mentality regeneration','magisters_hat', 9, [TAGS['item'], TAGS['accessory'],
                                                                             TAGS['night_vision'], TAGS['major_accessory']]),
    ...
    'demon_wings': Inventory.Item('Demon Wings', '100kg\n-200% speed\n-60% air resistance\n+50% wing control', 'demon_wings', 10,
                                    [TAGS['item'], TAGS['accessory'], TAGS['wings']]),
    'eternal_wings': Inventory.Item('Eternal Wings', '50kg\n+100% speed\n-50% air resistance\n50% wing control\n+30% dodge rate', 'eternal_wings', 11,
                                     [TAGS['item'], TAGS['accessory'], TAGS['wings']]),
    'eternal_pocket_watch': Inventory.Item('Eternal Pocket Watch', '+100% critical\n+50% damage absorb', 'eternal_pocket_watch', 11,
                                           [TAGS['item'], TAGS['accessory']]),
    ...
    'grasp_of_the_infinite_corridor': Inventory.Item('Grasp of the Infinite Corridor', 'rainbowA world, a planet, is all in my hand.\n+72% damage\n+180 touching defense\n+120 magic defense\n'
    ...
    'heart_locket': Inventory.Item('Heart Locket', 'You can feel it beating\n+300% speed\n+200% damage\n+100% critical\n'
                                                   ...
                                                   '+360% domain size\n+20/sec mentality regeneration', 'heart_locket', 12,
                                     [TAGS['item'], TAGS['accessory'], TAGS['major_accessory']]),
    ...
    'immune_potion': Inventory.Item('Immune Potion', 'Provides a about 2s immune time, but damage were dealt.', 'immune_potion', 4, [TAGS['item'], ]),
    'storm_potion': Inventory.Item('Storm Potion', 'Immediately repels nearby enemies.', 'storm_potion', 4, [TAGS['item'],]),
    'healing_potion': Inventory.Item('Healing Potion', 'Heals 150 HP', 'healing_potion', 2, [TAGS['item'], TAGS['healing_potion']]),
    'magic_potion': Inventory.Item('Magic Potion', 'Recover 120 MP', 'magic_potion', 2, [TAGS['item'], TAGS['magic_potion']]),
    'butterscotch_pie': Inventory.Item('Butterscotch Pie', 'Heals 240 HP', 'butterscotch_pie', 4,
                                       [TAGS['item'], TAGS['healing_potion']]),
    'seatea': Inventory.Item('Seatea', 'Recovers 180 MP', 'seatea', 4, [TAGS['item'], TAGS['magic_potion']]),
    ...
    'vitaflora_seed': Inventory.Item('Vitaflora Seed', '', 'vitaflora_seed', 2, [TAGS['item'], TAGS['herb_seed']]),
    'red_algae_seed': Inventory.Item('Red Algae Seed', '', 'red_algae_seed', 2, [TAGS['item'], TAGS['herb_seed']]),
    'anemovitis_seed': Inventory.Item('Anemovitis Seed', '', 'anemovitis_seed', 4, [TAGS['item'], TAGS['herb_seed']]),
    'magic_seed': Inventory.Item('Magic Seed', 'If it grows to something....', 'magic_seed', 2, [TAGS['item'], TAGS['herb_seed']]),
    ...
    'red_algae': Inventory.Item('Red Algae', '', 'red_algae', 2, [TAGS['item'], TAGS['herb']]),
    'anemovitis': Inventory.Item('Anemovitis', '', 'anemovitis', 4, [TAGS['item'], TAGS['herb']]),
    ...
    'spiritual_heart': Inventory.Item('Spiritual Heart',
                                      'Opens a new appearance of your world.',
                                      'spiritual_heart', 4, [TAGS['item']]),
    'scarlett_pearl': Inventory.Item('Scarlett Pearl', '+20 maximum hp (when HP >= 500)', 'scarlett_pearl', 5, [TAGS['item']]),
    'soul_prism': Inventory.Item('Soul Prism', '+30 maximum mana (when MP >= 320)', 'soul_prism', 5, [TAGS['item']]),
    'glimmer_stone': Inventory.Item('Glimmer Stone', '+25 maximum mana (when MP >= 500)', 'glimmer_stone', 6, [TAGS['item']]),
    'life_fruit': Inventory.Item('Life Fruit', '+60 maximum hp (when HP >= 640)\nWhen HP reaches 1000, your talent will come to a significant increase.', 'life_fruit', 7,
                                 [TAGS['item']]),
    'candlefire_rune': Inventory.Item('Candlefire Rune', '+100 maximum hp (when HP >= 1500)', 'candlefire_rune', 7, [TAGS['item']]),
    'saint_apple': Inventory.Item('Saint Apple', '+900 maximum HP\n700 maximum MP\n+20 maximum talent', 'saint_apple', 7,
                                 [TAGS['item']]),
    ...
    'delicious_seedling': Inventory.Item('Delicious Seedling', 'Summon python', 'delicious_seedling', 0, [TAGS['item'], TAGS['sacred']]),
    'suspicious_eye': Inventory.Item('Suspicious Eye', 'Summon the true eye', 'suspicious_eye', 0, [TAGS['item'], TAGS['sacred']]),
    'wild_fluffball': Inventory.Item('Wild Fluffball', 'Summon the fluffff', 'wild_fluffball', 0, [TAGS['item'], TAGS['sacred']]),
    ...
    'nature_core': Inventory.Item('Nature Core', 'Summon Glimmer Skate in fallen sea\nSummon Balrog in hell', 'nature_core', 0, [TAGS['item'], TAGS['sacred']]),
    ...
    'sky_painting': Inventory.Item('Sky Painting', 'Summon the sky tri-cubes', 'sky_painting', 0, [TAGS['item'], TAGS['sacred']]),
    'green_thing': Inventory.Item('Green Thing', 'Summon the heaven goblins.', 'green_thing', 0, [TAGS['item'], TAGS['sacred']]),
    'cold_present': Inventory.Item('Cold Present', 'Summon the "Everscream".', 'cold_present', 12, [TAGS['item'], TAGS['sacred']]),
    'mechanic_eye': Inventory.Item('Mechanic Eye', 'Summon the twin eyes', 'mechanic_eye', 0, [TAGS['item'], TAGS['sacred']]),
    'mechanic_worm': Inventory.Item('Mechanic Worm', 'Summon the destroyer', 'mechanic_worm', 0, [TAGS['item'], TAGS['sacred']]),
    'electric_unit': Inventory.Item('Electric Unit', 'Summon the CPU', 'electric_unit', 0, [TAGS['item'], TAGS['sacred']]),
    'mechanic_spider': Inventory.Item('Electric Spider', 'Summon the Greed', 'mechanic_spider', 0, [TAGS['item'], TAGS['sacred']]),
    'watch': Inventory.Item('Watch', 'Summon the eye of time', 'watch', 0, [TAGS['item'], TAGS['sacred']]),
    'metal_food': Inventory.Item('Metal Food', 'Summon the devil python', 'metal_food', 0, [TAGS['item'], TAGS['sacred']]),
    'joker': Inventory.Item('JOKER', 'Summon the Joker Evil', 'joker', 0, [TAGS['item'], TAGS['sacred']]),
    'plantera_bulb': Inventory.Item('Plantera Bulb', 'Summon Plantera', 'plantera_bulb', 0, [TAGS['item'], TAGS['sacred']]),
    'yellow_flower': Inventory.Item('Yellow Flower', 'Final.', 'yellow_flower', 12, [TAGS['item'], TAGS['sacred']]),
...
RECIPES = [
    ...
    Recipe({'zirconium_ingot': 30, 'magic_stone': 5, 'anvil': 1}, 'submachine_gun'),
    ...
    Recipe({'cauldron': 1, 'dendrobium': 3, 'crysanths': 1, 'platinum': 5}, 'shield_potion'),
    Recipe({'cauldron': 1, 'dendrobium': 3, 'crysanths': 1, 'zirconium': 5}, 'shield_potion'),
    Recipe({'cauldron': 1, 'dendrobium': 1, 'flamaureus': 2, 'winteraceae': 1, 'blood_ingot': 1}, 'wrath_potion'),
    Recipe({'cauldron': 1, 'firy_plant': 2, 'red_algae': 2, 'firite_ingot': 1}, 'healing_potion', 2),
    ...
    Recipe({'firite_ingot': 20, 'aerialite_ingot': 10, 'leaf': 10, 'mana_crystal': 7, 'sea_pearl': 1, 'anvil': 1}, 'lily_of_the_valley'),
    Recipe({'aerialite_ingot': 13, 'anvil': 1}, 'sky_edge'),
    ...
    Recipe({'aerialite_ingot': 13, 'anvil': 1}, 'cloudcore_staff'),
    Recipe({'obsidian_ingot': 28, 'anvil': 1}, 'obsidian_sword'),
    Recipe({'obsidian_ingot': 25, 'anvil': 1}, 'obsidian_polearm'),
    ...
    Recipe({'mysterious_ingot': 32, 'obsidian_ingot': 18, 'anvil': 1}, 'grenade_kings_jade_mask'),
    ...
    Recipe({'bullet': 50, 'forgotten_shard': 2, 'anvil': 1}, 'wavic', 50),
    Recipe({'forgotten_shard': 20, 'coral_reef': 30, 'submachine_gun': 1, 'anvil': 1}, 'minishark'),
    Recipe({'forgotten_shard': 30, 'coral_reef': 20, 'sea_pearl': 1, 'anvil': 1}, 'minishark'),
    Recipe({'dangerous_necklace': 1, 'fearful_necklace': 1, 'bloodstone': 4, 'anvil': 1}, 'war_necklace'),
    Recipe({'life_flower': 1, 'mana_flower': 1, 'bloodstone': 4, 'anvil': 1}, 'bloodflower'),
    Recipe({'tearblade': 1, 'magic_sword': 1, 'dim_heavysword': 1, 'volcano': 1, 'storm_core': 1},
           'nights_edge'),
    ...
    Recipe({'soul': 15, 'mana_crystal': 3, 'forgotten_shard': 4}, 'soul_prism'),
    ...
    Recipe({'star_amulet': 1, 'soul_of_flying': 15, 'mithrill_anvil': 1}, 'angels_amulet'),
    Recipe({'minishark': 1, 'soul_of_fire': 20, 'firite_ingot': 20, 'biome_alloy': 5,
            'mithrill_anvil': 1}, 'lavashark'),
    Recipe({'red_ring': 1, 'palladium_ingot': 20, 'mithrill_ingot': 20, 'titanium_ingot': 20,
            'soul': 50, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'red_spirit_ring'),
    Recipe({'titanium_ingot': 15, 'mithrill_ingot': 20, 'mithrill_anvil': 1}, 'mechanic_boots'),
    Recipe({'spiritual_stabber': 1, 'evil_ingot': 20, 'soul': 120, 'mithrill_anvil': 1}, 'balanced_stabber'),
    Recipe({'saint_steel_ingot': 10, 'life_core': 5, 'titanium_ingot': 20, 'soul_of_growth': 10, 'biome_alloy': 5,
    ...
    Recipe({'soul': 30, 'palladium_ingot': 8, 'evil_ingot': 15, 'soul_of_fire': 10, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'blood_sacrifice'),
    Recipe({'nameless_fire': 1, 'soul': 50, 'soul_of_fire': 20, 'evil_ingot': 10, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'karmic_fire'),
    ...
    Recipe({'obsidian_ingot': 1, 'aerialite_ingot': 1, 'mysterious_ingot': 1, 'mithrill_anvil': 1}, 'biome_alloy'),
    Recipe({'obsidian_ingot': 2, 'firy_plant': 1, 'soul_of_fire': 3, 'soul': 3, 'mithrill_anvil': 1}, 'thermal_ingot'),
    Recipe({'balanced_stabber': 1, 'saint_steel_ingot': 8, 'biome_alloy': 12, 'mithrill_anvil': 1}, 'excalibur'),
    Recipe({'magic_sword': 1, 'saint_steel_ingot': 4, 'soul_of_coldness': 15, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'glacier'),
    Recipe({'thermal_ingot': 15, 'mithrill_anvil': 1}, 'thermal_pike'),
    Recipe({'thermal_ingot': 15, 'mithrill_anvil': 1}, 'fires_defense'),
    Recipe({'thermal_ingot': 18, 'starlight_shard': 2, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'fire_shine'),
    Recipe({'bullet': 80, 'soul_of_fire': 1, 'thermal_ingot': 2, 'mithrill_anvil': 1}, 'thermality', 80),
    Recipe({'saint_steel_ingot': 2, 'daedalus_ingot': 1, 'soul': 6, 'mithrill_anvil': 1}, 'judgement', 80),
    Recipe({'windstorm_swordman_mark': 1, 'saint_steel_ingot': 5, 'mithrill_anvil': 1}, 'paladins_mark'),
    Recipe({'discord_storm': 1, 'daedalus_ingot': 8, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'daedalus_stormbow'),
    Recipe({'daedalus_ingot': 12, 'mithrill_anvil': 1}, 'daedalus_knife'),
    Recipe({'daedalus_knife': 1, 'spiritual_knife': 1, 'biome_alloy': 5, }, 'daedalus_twinknife'),
    Recipe({'wings': 1, 'biome_alloy': 5, 'starlight_shard': 5}, 'star_wings'),
    Recipe({'terrified_necklace': 1, 'biome_alloy': 5, 'starlight_shard': 5}, 'guardian_necklace'),
    Recipe({'copper_traveller_boots': 1, 'starlight_shard': 12}, 'broken_copper_boots'),
    Recipe({'windstorm_assassin_mark': 1, 'daedalus_ingot': 5, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'daedalus_mark'),
    ...
    Recipe({'soul_bottle': 1, 'soul_resonancer': 1, 'soul_of_integrity': 18, 'biome_alloy': 5, 'mithrill_anvil': 1}, 'unregretted_heart'),
    Recipe({'palladium_ingot': 24, 'soul_of_integrity': 10, 'mithrill_anvil': 1}, 'gravity_wand'),
    Recipe({'wings': 1, 'soul_of_integrity': 10, 'biome_alloy': 12, 'mithrill_anvil': 1}, 'honest_flyer'),
    Recipe({'soul_of_integrity': 2, 'soul': 1, 'mithrill_anvil': 1}, 'quick_arrow', 60),
    Recipe({'daedalus_ingot': 6, 'soul_of_bravery': 20, 'mithrill_anvil': 1}, 'tough_gloves'),
    Recipe({'soul_of_bravery': 2, 'soul': 1, 'mithrill_anvil': 1}, 'quick_bullet', 60),
    Recipe({'mechanic_boots': 1, 'soul_of_bravery': 15, 'soul': 25, 'mithrill_anvil': 1}, 'highspeed_boots'),
    Recipe({'mithrill_ingot': 12, 'soul_of_bravery': 18, 'soul_of_integrity': 12, 'biome_alloy': 12, 'mithrill_anvil': 1},
           'forward_bow'),
    Recipe({'mithrill_ingot': 24, 'soul_of_kindness': 10, 'mithrill_anvil': 1}, 'shield_wand'),
    Recipe({'saint_steel_ingot': 12, 'soul_of_bravery': 10, 'soul_of_integrity': 15, 'biome_alloy': 12, 'mithrill_anvil': 1},
           'guardian'),
    Recipe({'mithrill_ingot': 20, 'soul_of_kindness': 24}, 'energy_spear'),
    Recipe({'dark_ingot': 6, 'soul_of_kindness': 20}, 'burnt_pan'),
    Recipe(
        {'excalibur': 1, 'soul_of_integrity': 1, 'soul_of_bravery': 1, 'soul_of_kindness': 1, 'spirit_essence': 3, 'mithrill_anvil': 1},
        'true_excalibur'),
    Recipe(
        {'nights_edge': 1, 'soul_of_integrity': 1, 'soul_of_bravery': 1, 'soul_of_kindness': 1, 'spirit_essence': 3, 'mithrill_anvil': 1},
        'true_nights_edge'),
    Recipe(
        {'saint_steel_ingot': 24, 'evil_ingot': 12, 'biome_alloy': 12, 'spirit_essence': 3,
         'mithrill_anvil': 1}, 'muramasa'),
    Recipe(
        {'saint_steel_ingot': 16, 'evil_ingot': 12, 'biome_alloy': 12, 'mystery_core': 4, 'spirit_essence': 3,
         'mithrill_anvil': 1}, 'forbidden_oath'),
    Recipe({'shadow': 1, 'soul_of_integrity': 1, 'soul_of_bravery': 1, 'soul_of_kindness': 1, 'spirit_essence': 3, 'mithrill_anvil': 1},
           'true_shadow'),
    Recipe({'daedalus_twinknife': 1, 'soul_of_integrity': 1, 'soul_of_bravery': 1, 'soul_of_kindness': 1, 'spirit_essence': 3, 'mithrill_anvil': 1},
           'true_twinblade'),
    Recipe({'soul_of_integrity': 2, 'soul_of_bravery': 1, 'soul_of_kindness': 1, 'mana_crystal': 3},
           'mystery_core'),
    Recipe({'soul_of_integrity': 3, 'soul_of_bravery': 3, 'soul_of_kindness': 3, 'life_core': 1, 'starlight_shard': 1},
           'spirit_essence'),
    Recipe({'mystery_core': 2, 'mana_crystal': 5, 'soul_of_patience': 2},
           'glimmer_stone'),
    Recipe({'mystery_core': 1, 'spirit_essence': 1, 'soul_of_integrity': 2, 'cauldron': 1}, 'tension_bit', 3),
    Recipe({'mithrill_ingot': 30, 'mystery_core': 3, 'spirit_essence': 3, 'soul': 50}, 'forbidden_curse__spirit'),
    Recipe({'mithrill_ingot': 30, 'mystery_core': 3, 'spirit_essence': 3, 'evil_ingot': 20}, 'forbidden_curse__evil'),
    Recipe({'glacier': 1, 'mantle': 1, 'soul_of_coldness': 15, 'soul_of_fire': 15, 'biome_alloy': 15, 'spirit_essence': 3,
            'mithrill_anvil': 1}, 'fiery_iceberg'),
    Recipe({'sky_edge': 1, 'soul_of_coldness': 25, 'biome_alloy': 18, 'spirit_essence': 3, 'mithrill_anvil': 1}, 'ice_crystal_edge'),
    Recipe({'daedalus_stormbow': 1, 'soul_of_integrity': 1, 'soul_of_bravery': 1, 'soul_of_kindness': 1, 'spirit_essence': 1,
            'mithrill_anvil': 1}, 'true_daedalus_stormbow'),
    Recipe({'soul_of_perseverance': 16, 'biome_alloy': 10}, 'perseverance_sword'),
    Recipe({'soul_of_perseverance': 18, 'biome_alloy': 10}, 'black_hole_sword'),
    Recipe({'soul_of_perseverance': 16, 'biome_alloy': 10}, 'worn_notebook'),
    Recipe({'mithrill_ingot': 30, 'mystery_core': 3, 'spirit_essence': 3, 'soul_of_patience': 10}, 'forbidden_curse__time'),
    Recipe({'dark_spider_lily': 1, 'biome_alloy': 10, 'mystery_core': 2, 'spirit_essence': 3, 'soul_of_patience': 15}, 'time_lily'),
    Recipe({'soul_of_patience': 16, 'biome_alloy': 10}, 'toy_knife'),
    Recipe({'soul_of_patience': 8, 'soul_of_perseverance': 12, 'biome_alloy': 10, 'spirit_essence': 2}, 'millennium_persists'),
    Recipe({'soul_of_justice': 16, 'biome_alloy': 10}, 'empty_gun'),
    Recipe({'shotgun': 1, 'biome_alloy': 10, 'soul_of_justice': 18, 'soul_of_bravery': 8, 'spirit_essence': 1},
           'justice_shotgun'),
    Recipe({'aimer': 1, 'daedalus_ingot': 6, 'biome_alloy': 6, 'soul_of_justice': 18, 'soul_of_patience': 8, 'spirit_essence': 1},
           'critically'),
    Recipe({'phoenix_exploder': 1, 'biome_alloy': 16, 'soul_of_coldness': 30,
            'soul_of_justice': 15, 'soul_of_bravery': 15, 'spirit_essence': 2}, 'frozen_fire_exploder'),
    Recipe({'soul_of_justice': 16, 'biome_alloy': 10}, 'bow_of_sanction'),
    Recipe({'soul_of_flying': 1, 'soul_of_growth': 1, 'soul_of_coldness': 1}, 'soul', 7),
    Recipe({'spirit_essence': 1}, 'soul', 7),
    Recipe({'sky': 1, 'biome_alloy': 12, 'soul_of_patience': 12, 'spirit_essence': 2}, 'celestic'),

    Recipe({'chlorophyll': 1, 'soul_of_patience': 2, 'soul_of_perseverance': 2, 'soul_of_justice': 2, 'photon': 3}, 'incremental_spirit_essence'),
    ...
    Recipe({'dark_ingot': 10, 'photon': 20, 'spirit_essence': 3}, 'prism'),
    Recipe({'life_wooden_sword': 1, 'photon': 15, 'incremental_spirit_essence': 2}, 'incremental_broadsword'),
    Recipe({'chlorophyll': 1, 'biome_alloy': 1, 'photon': 3, 'soul_of_growth': 3,
            'life_core': 1, 'incremental_spirit_essence': 1}, 'chlorophyte_ingot'),
    Recipe({'chlorophyll': 1, 'biome_alloy': 3, 'photon': 7, 'soul_of_growth': 7,
            'life_core': 2, 'incremental_spirit_essence': 1}, 'chlorophyte_ingot', 3),
    Recipe({'chlorophyte_ingot': 16, 'soul_of_perseverance': 18, 'leaf': 50, 'incremental_spirit_essence': 3},
           'life_devourer'),
    Recipe({'chlorophyte_ingot': 18, 'soul_of_perseverance': 15, 'soul_of_growth': 25, 'leaf': 80, 'incremental_spirit_essence': 2},
           'leavigour'),
    Recipe({'chlorophyte_ingot': 24, 'soul_of_justice': 18, 'photon': 20, 'leaf': 20, 'incremental_spirit_essence': 3},
           'lazer_rain'),
    Recipe({'chlorophyte_ingot': 18, 'soul_of_justice': 18, 'soul_of_perseverance': 6, 'leaf': 20, 'incremental_spirit_essence': 2},
           'lazer_gun'),
    Recipe({'chlorophyte_ingot': 32, 'soul_of_justice': 12, 'soul_of_perseverance': 12, 'leaf': 20, 'incremental_spirit_essence': 2},
           'lazer_sniper'),
    Recipe({'chlorophyte_ingot': 8, 'photon': 20, 'soul_of_growth': 45, 'leaf': 50, 'incremental_spirit_essence': 3}, 'growth'),
    Recipe({'prism': 1, 'chlorophyte_ingot': 12, 'photon': 30, 'incremental_spirit_essence': 2}, 'prism_wand'),
    ...
    Recipe({'chlorophyll': 1, 'world_bow': 1, 'biome_alloy': 10, 'chlorophyte_ingot': 10, 'photon': 30, 'incremental_spirit_essence': 3}, 'true_worlds_bow'),
    Recipe({'chlorophyte_ingot': 2, 'photon': 5}, 'chloro_arrow', 40),
    Recipe({'bullet': 40, 'chlorophyte_ingot': 2, 'photon': 5}, 'chlorommo', 40),
    Recipe({'chlorophyte_ingot': 16, 'biome_alloy': 8, 'incremental_spirit_essence': 5}, 'necklace_of_life'),
    Recipe({'chlorophyte_ingot': 16, 'biome_alloy': 8, 'incremental_spirit_essence': 5}, 'thorn_ring'),
    Recipe({'soul_of_growth': 12, 'chlorophyte_ingot': 1, 'life_core': 5, 'incremental_spirit_essence': 1}, 'life_fruit'),
    Recipe({'aimer': 1, 'photon': 32, 'incremental_spirit_essence': 2}, 'photon_aimer'),
    Recipe({'soul_of_growth': 24, 'chlorophyte_ingot': 8, 'incremental_spirit_essence': 5}, 'life_wand'),
    Recipe({'chlorophyte_ingot': 5, 'soul_of_justice': 20, 'incremental_spirit_essence': 6}, 'cowboy_hat'),
    Recipe({'chlorophyte_ingot': 5, 'soul_of_perseverance': 20, 'incremental_spirit_essence': 6}, 'cloudy_glasses'),
    Recipe({'chlorophyte_ingot': 5, 'soul_of_patience': 20, 'incremental_spirit_essence': 6}, 'magicians_hat'),
    Recipe({'fairy_boots': 1, 'wing_boots': 1, 'chlorophyte_ingot': 12, 'photon': 24, 'incremental_spirit_essence': 2}, 'life_boots'),
    Recipe({'natural_necklace': 1, 'seed_amulet': 1, 'bloodstone_amulet': 1,
            'incremental_spirit_essence': 6, 'spirit_essence': 6}, 'naturalify_necklace'),
    Recipe({'chaos_ingot': 1}, 'space_jumper', 20),
    Recipe({'chaos_ingot': 3}, 'space_jumper', 100),
    Recipe({'chaos_ingot': 32}, 'chaos_teleporter'),
    Recipe({'chaos_ingot': 32}, 'chaos_killer'),
    Recipe({'chaos_ingot': 32, 'incremental_spirit_essence': 3}, 'gods_necklace'),
    Recipe({'chaos_ingot': 12, 'firite_ingot': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'skyfire__meteor'),
    Recipe({'chaos_ingot': 12, 'magic_stone': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'azure_guard'),
    Recipe({'chaos_ingot': 12, 'storm_core': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'storm'),
    Recipe({'chaos_ingot': 12, 'iron_ingot': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'earth_wall'),
    Recipe({'chaos_ingot': 15, 'life_core': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'lifebringer'),
    Recipe({'chaos_ingot': 15, 'cell_organization': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'target_dummy'),
    Recipe({'chaos_ingot': 15, 'photon': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'judgement_light'),
    Recipe({'chaos_ingot': 1, 'dark_ingot': 1, 'mystery_core': 2, 'spirit_essence': 3}, 'dark_restrict'),
    Recipe({'chaos_ingot': 32, 'firite_ingot': 50, 'mystery_core': 2, 'spirit_essence': 5,
            'incremental_spirit_essence': 3}, 'great_forbidden_curse__fire'),
    Recipe({'chaos_ingot': 32, 'photon': 100, 'mystery_core': 2, 'spirit_essence': 5,
            'incremental_spirit_essence': 3}, 'great_forbidden_curse__light'),
    Recipe({'true_nights_edge': 1, 'true_excalibur': 1, 'jevil_knife': 1, 'incremental_spirit_essence': 4}, 'the_blade'),
    Recipe({'biome_alloy': 24, 'spirit_essence': 8, 'incremental_spirit_essence': 4}, 'wilson_knife'),
    Recipe({'muramasa': 1, 'forbidden_oath': 1, 'chaos_ingot': 12, 'incremental_spirit_essence': 4}, 'demon_blade__muramasa'),
    Recipe({'chaos_ingot': 30, 'jevil_knife': 1, 'incremental_spirit_essence': 6}, 'chaos_chaos'),
    Recipe({'chaos_ingot': 26}, 'chaos_abyss'),
    Recipe({'chaos_ingot': 20}, 'chaos_assaulter'),
    Recipe({'chaos_ingot': 30, 'frozen_fire_exploder': 1, 'incremental_spirit_essence': 6, 'spirit_essence': 4}, 'chaos_annihilator'),
    Recipe({'chaos_ingot': 40, 'soul_of_patience': 20, 'spirit_essence': 7}, 'great_magicians_hat'),
    Recipe({'chaos_ingot': 40, 'spirit_essence': 7, 'quiver': 1}, 'chaos_quiver'),
    ...
    Recipe({'soul': 4, 'time_essence': 4, 'substance_essence': 4, 'wierd_essence': 3, 'light_essence': 3}, 'origin_spirit_essence'),
    Recipe({'willpower_shard': 2, 'light_essence': 5, 'origin': 1}, 'candlefire_rune'),
    Recipe({'seed_of_origin': 999, 'soul_of_determination': 9}, 'origin'),
    ...
    Recipe({'demolisher': 1, 'chaos_annihilator': 1}, 'void'),
    Recipe({'fiery_iceberg': 1, 'gaias_palladin_spear': 1, 'soul_of_determination': 36, 'origin': 1}, 'ark_of_elements'),
    Recipe({'time_lily': 1, 'lily_of_the_valley': 1, 'soul_of_determination': 36, 'origin': 1}, 'dream_violet'),
    Recipe({'curse_book': 1, 'growth': 1, 'soul_of_determination': 36, 'origin': 1}, 'forbidden_curse__life'),
    Recipe({'guardian': 1, 'cosmos': 1, 'soul_of_determination': 16, 'origin': 1}, 'star_defense'),
    ...
    Recipe({'chaos_ingot': 400, 'soul_of_determination': 20, 'origin_feather': 10, 'origin_spirit_essence': 3}, 'eternal_wings'),
    Recipe({'chaos_ingot': 400, 'soul_of_determination': 20, 'origin_feather': 10, 'origin_spirit_essence': 3}, 'eternal_pocket_watch'),
    ...
    Recipe({'chaos_ingot': 50, 'soul_of_determination': 20,
            'wings': 1, 'fairy_wings': 1, 'snow_wings': 1, 'star_wings': 1,
            'origin_spirit_essence': 5}, 'altitude'),
    ...
    Recipe({'chaos_ingot': 250, 'soul_of_determination': 16, 'light_essence': 54,
            'chaos_evileye': 1, 'origin_spirit_essence': 5}, 'horizon_goggles'),
    ...
    Recipe({'bruning_tears': 1, 'thermal_pike': 1, 'soul_of_determination': 16, 'origin_feather': 4, 'light_essence': 30, 'celestial_fountain': 1, 'time_fountain': 1, 'origin_spirit_essence': 3}, 'eternal_sunfire'),
    Recipe({'chaos_ingot': 600, 'soul_of_determination': 12, 'origin_feather': 5, 'light_essence': 25, 'celestial_fountain': 1, 'time_fountain': 1, 'origin_spirit_essence': 3}, 'ember'),
    Recipe({'chaos_ingot': 600, 'soul_of_determination': 16, 'origin_feather': 8, 'time_essence': 30, 'time_fountain': 1, 'death_fountain': 1}, 'retribution'),
    Recipe({'chaos_ingot': 30, 'soul_of_determination': 1, 'light_essence': 2}, 'scorching_arrow', 20),
    Recipe({'chaos_ingot': 50, 'soul_of_determination': 3, 'time_essence': 3, 'origin_feather': 1, 'origin_spirit_essence': 1}, 'eterrow', 100),
    Recipe({'chaos_ingot': 30, 'soul_of_determination': 1, 'substance_essence': 2, 'substance_fountain': 1}, 'seperator', 20),
    ...
    Recipe({'soul_of_determination': 25, 'time_fountain': 1, 'substance_fountain': 1, 'celestial_fountain': 1,
            'spirit_essence': 10, 'incremental_spirit_essence': 10, 'origin_spirit_essence': 20}, 'my_soul'),
    Recipe({'my_soul': 1, 'wings': 1, 'origin_spirit_essence': 3}, 'rudder'),
    Recipe({'chaos_ingot': 200, 'soul_of_determination': 20, 'my_soul': 1,
            'time_fountain': 1, 'substance_fountain': 1, 'celestial_fountain': 1,
            'origin_spirit_essence': 5}, 'destruct_thoughts'),
    Recipe({'copper_ingot': 1, 'iron_ingot': 1, 'cobalt_ingot': 1, 'steel_ingot': 1, 'silver_ingot': 1,
            ...
            'firite_ingot': 1, 'aerialite_ingot': 1, 'mysterious_ingot': 1, 'evil_ingot': 1, 'palladium_ingot': 1, 'mithrill_ingot': 1,
            'titanium_ingot': 1, 'thermal_ingot': 1, 'chlorophyte_ingot': 1, 'chaos_ingot': 1, 'wooden_hammer': 1,
            'anvil': 1, 'furnace': 1, 'mithrill_anvil': 1, 'chlorophyll': 1, 'time_fountain': 1,
            'substance_fountain': 1, 'celestial_fountain': 1, 'my_soul': 1, 'reason': 9, 'result': 9},
           'the_final_ingot', 9),
    Recipe({'my_soul': 1, 'spirit_essence': 2, 'incremental_spirit_essence': 2, 'origin_spirit_essence': 3}, 'ascendant_spirit_essence'),
    Recipe({'the_final_ingot': 32, 'determined_ring': 1, 'my_soul': 1, 'ascendant_spirit_essence': 2}, 'my_ring'),
    ...
    Recipe({'galaxy_broadsword': 1, 'my_soul': 1, 'death_fountain': 1, 'the_final_ingot': 6, 'ascendant_spirit_essence': 1}, 'eternal_echo'),
    Recipe({'galaxy_broadsword': 1, 'my_soul': 1, 'no_fountain': 1, 'the_final_ingot': 6, 'ascendant_spirit_essence': 1}, 'star_of_devotion'),
    Recipe({'rising_action': 1, 'my_soul': 1, 'death_fountain': 1, 'the_final_ingot': 6, 'ascendant_spirit_essence': 1}, 'relevation_of_cycles'),

    Recipe({'spiritual_destroyer': 1, 'leavigour': 1, 'the_final_ingot': 5, 'my_soul': 1, 'death_fountain': 1, 'ascendant_spirit_essence': 2}, 'mind'),
    Recipe({'excalibur': 1, 'black_hole_sword': 1, 'millennium_persists': 1, 'the_final_ingot': 5, 'my_soul': 1, 'no_fountain': 1, 'ascendant_spirit_essence': 2}, 'adherent'),
    Recipe({'the_blade': 1, 'seedler': 1, 'cosmos': 1, 'the_final_ingot': 5, 'my_soul': 1, 'no_fountain': 1, 'ascendant_spirit_essence': 2}, 'worlds_lord'),

    Recipe({'altitude': 1, 'demon_wings': 1, 'time_wheel': 1, 'black_hole_pluvial': 1, 'matter_floatboard': 1,
            'daedalus_mark': 1, 'soul_of_flying': 99, 'my_soul': 1, 'the_final_ingot': 5, 'ascendant_spirit_essence': 3}, 'ultra_lightspeed'),
    Recipe({'ultra_lightspeed': 1, 'the_final_ingot': 5, 'soulfeather': 81, 'my_soul': 1, 'ascendant_spirit_essence': 6}, 'beyond_horizon'),
    Recipe({'starnight_gloves': 1, 'flashback': 1, 'matters_touch': 1, 'thorn_ring': 1, 'my_ring': 1,
            'origin': 9, 'my_soul': 1, 'the_final_ingot': 5, 'ascendant_spirit_essence': 3}, 'grasp_of_the_infinite_corridor'),
    Recipe({'corrupt_boots': 1, 'ultradense_shield': 1, 'time_shackle': 1, 'magic_anklet': 1,
            'rudder': 1, 'mysterious_ingot': 99, 'my_soul': 1, 'the_final_ingot': 5, 'ascendant_spirit_essence': 3}, 'karmic_trail_boots'),
    Recipe({'the_chaos_flower_vase': 1, 'horizon_goggles': 1, 'flashback': 1, 'demon_contract': 1, 'seed_amulet': 1,
            'lucky_clover': 9, 'my_soul': 1, 'the_final_ingot': 5, 'ascendant_spirit_essence': 3}, 'fate_alignment_amulet'),

    Recipe({'copper_sword': 1, 'doctor_expeller': 1, 'demon_blade__muramasa': 1, 'the_blade': 1, 'uncanny_valley': 1,
            'star_wrath': 1, 'lysis': 1, 'galaxy_broadsword': 1, 'turning_point': 1, 'the_final_soul': 20, 'ascendant_spirit_essence': 9}, 'zenith'),
    Recipe({'bravery_amulet': 1, 'kindness_amulet': 1, 'integrity_amulet': 1, 'perseverance_amulet': 1,
            'patience_amulet': 1, 'justice_amulet': 1, 'the_final_ingot': 10, 'my_soul': 1, 'the_final_soul': 20, 'ascendant_spirit_essence': 9}, 'heart_locket'),
    Recipe({'the_final_ingot': 6, 'time_fountain': 1, 'substance_fountain': 1, 'celestial_fountain': 1,
            'death_fountain': 1, 'no_fountain': 1, 'my_soul': 1, 'plantera_bulb': 1, 'ascendant_spirit_essence': 3}, 'yellow_flower'),
    ...
    Recipe({'copper_ingot': 18, 'anvil': 1}, 'copper_helmet'),
    Recipe({'copper_ingot': 18, 'anvil': 1}, 'copper_mask'),
    ...
    Recipe({'iron_ingot': 24, 'anvil': 1}, 'iron_chestplate'),
    Recipe({'iron_ingot': 18, 'anvil': 1}, 'iron_greaves'),

    Recipe({'cobalt_ingot': 12, 'anvil': 1}, 'cobalt_helmet'),
    Recipe({'cobalt_ingot': 12, 'anvil': 1}, 'cobalt_mask'),
    Recipe({'cobalt_ingot': 24, 'anvil': 1}, 'cobalt_battlegear'),
    Recipe({'cobalt_ingot': 18, 'anvil': 1}, 'cobalt_leggings'),

    Recipe({'steel_ingot': 12, 'anvil': 1}, 'steel_helmet'),
    Recipe({'steel_ingot': 12, 'anvil': 1}, 'steel_mask'),
    Recipe({'steel_ingot': 24, 'anvil': 1}, 'steel_chestplate'),
    Recipe({'steel_ingot': 18, 'anvil': 1}, 'steel_greaves'),

    Recipe({'silver_ingot': 12, 'anvil': 1}, 'silver_helmet'),
    Recipe({'silver_ingot': 12, 'anvil': 1}, 'silver_mask'),
    Recipe({'silver_ingot': 24, 'anvil': 1}, 'silver_battlegear'),
    Recipe({'silver_ingot': 18, 'anvil': 1}, 'silver_leggings'),

    Recipe({'platinum_ingot': 11, 'anvil': 1}, 'platinum_helmet'),
    Recipe({'platinum_ingot': 11, 'anvil': 1}, 'platinum_mask'),
    Recipe({'platinum_ingot': 21, 'anvil': 1}, 'platinum_chestplate'),
    Recipe({'platinum_ingot': 16, 'anvil': 1}, 'platinum_greaves'),

    Recipe({'zirconium_ingot': 11, 'anvil': 1}, 'zirconium_helmet'),
    Recipe({'zirconium_ingot': 11, 'anvil': 1}, 'zirconium_mask'),
    Recipe({'zirconium_ingot': 21, 'anvil': 1}, 'zirconium_chestplate'),
    Recipe({'zirconium_ingot': 16, 'anvil': 1}, 'zirconium_greaves'),

    Recipe({'blood_ingot': 8, 'anvil': 1}, 'bloody_helmet'),
    Recipe({'blood_ingot': 8, 'anvil': 1}, 'bloody_mask'),
    Recipe({'blood_ingot': 18, 'anvil': 1}, 'bloody_chestplate'),
    Recipe({'blood_ingot': 14, 'anvil': 1}, 'bloody_greaves'),

    Recipe({'blood_ingot': 7, 'bloodstone': 3, 'anvil': 1}, 'bloodstone_mask'),
    Recipe({'blood_ingot': 11, 'bloodstone': 5, 'anvil': 1}, 'bloodstone_cloak'),

    Recipe({'firite_ingot': 17, 'anvil': 1}, 'firy_helmet'),
    Recipe({'firite_ingot': 17, 'anvil': 1}, 'firy_mask'),
    Recipe({'firite_ingot': 28, 'anvil': 1}, 'firy_chestplate'),
    Recipe({'firite_ingot': 22, 'anvil': 1}, 'firy_greaves'),
    ...
    Recipe({'aerialite_ingot': 27, 'floatstone': 12, 'anvil': 1}, 'aerialite_cloak'),
    Recipe({'aerialite_ingot': 18, 'floatstone': 6, 'anvil': 1}, 'aerialite_boots'),

    Recipe({'forgotten_shard': 20, 'coral_reef': 10, 'anvil': 1}, 'forgotten_headgear'),
    Recipe({'forgotten_shard': 25, 'coral_reef': 12, 'anvil': 1}, 'forgotten_traveller_cloak'),
    Recipe({'forgotten_shard': 18, 'coral_reef': 7, 'anvil': 1}, 'forgotten_heavyboots'),

    Recipe({'mysterious_ingot': 4, 'anvil': 1}, 'mysterious_helmet'),
    Recipe({'mysterious_ingot': 4, 'anvil': 1}, 'mysterious_mask'),
    Recipe({'mysterious_ingot': 11, 'anvil': 1}, 'mysterious_chestplate'),
    Recipe({'mysterious_ingot': 7, 'anvil': 1}, 'mysterious_greaves'),

    Recipe({'mysterious_ingot': 3, 'storm_core': 1, 'anvil': 1}, 'storm_eye_mask'),
    Recipe({'mysterious_ingot': 8, 'storm_core': 1, 'anvil': 1}, 'storm_eye_chestplate'),
    Recipe({'mysterious_ingot': 5, 'storm_core': 1, 'anvil': 1}, 'storm_eye_greaves'),

    Recipe({'forgotten_shard': 20, 'coral_reef': 15, 'sea_pearl': 2, 'anvil': 1}, 'tidal_mask'),
    Recipe({'forgotten_shard': 25, 'coral_reef': 18, 'sea_pearl': 3, 'anvil': 1}, 'tidal_chestplate'),
    Recipe({'forgotten_shard': 18, 'coral_reef': 12, 'sea_pearl': 2, 'anvil': 1}, 'tidal_greaves'),

    Recipe({'titanium_ingot': 7, 'palladium_ingot': 7, 'mithrill_anvil': 1}, 'merged_metal_helmet'),
    Recipe({'palladium_ingot': 7, 'mithrill_ingot': 7, 'mithrill_anvil': 1}, 'merged_metal_mask'),
    Recipe({'mithrill_ingot': 7, 'titanium_ingot': 7, 'mithrill_anvil': 1}, 'merged_metal_headgear'),
    Recipe({'palladium_ingot': 16, 'mithrill_ingot': 16, 'titanium_ingot': 16, 'mithrill_anvil': 1}, 'merged_metal_chestplate'),
    Recipe({'palladium_ingot': 5, 'mithrill_ingot': 5, 'titanium_ingot': 5, 'mithrill_anvil': 1}, 'merged_metal_greaves'),

    Recipe({'thermal_ingot': 12, 'soul_of_bravery': 12, 'soul_of_fire': 18, 'mithrill_anvil': 1}, 'everscorch_helmet'),
    Recipe({'thermal_ingot': 16, 'soul_of_bravery': 16, 'soul_of_fire': 24, 'mithrill_anvil': 1}, 'everscorch_chestplate'),
    Recipe({'thermal_ingot': 14, 'soul_of_bravery': 14, 'soul_of_fire': 21, 'mithrill_anvil': 1}, 'everscorch_greaves'),

    Recipe({'dark_ingot': 18, 'soul_of_integrity': 12, 'starnight_shard': 1, 'mithrill_anvil': 1}, 'stardream_hood'),
    ...
    Recipe({'chaos_ingot': 40, 'willpower_shard': 3}, 'godkiller_helmet'),
    Recipe({'chaos_ingot': 60, 'willpower_shard': 5}, 'godkiller_chestplate'),
    Recipe({'chaos_ingot': 40, 'willpower_shard': 2}, 'godkiller_leggings'),
    ...
    Recipe({'the_final_ingot': 7, 'my_soul': 1}, 'karmic_leggings'),
    ...
    Recipe({'hallow_ingot': 10}, 'hallowed_greaves'),
    ...
    Recipe({'cauldron': 1, 'gypsophila_seed': 10, 'dendrobium_seed': 10, 'storm_core': 1}, 'anemovitis_seed', 10),
    Recipe({'cauldron': 1, 'anemovitis': 1}, 'anemovitis_seed', 2),
    ...
    Recipe({'life_core': 2, 'leaf': 20}, 'red_apple'),
    Recipe({'platinum_ingot': 10, 'floatstone': 25}, 'monument'),
    Recipe({'gypsophila': 10, 'winteraceae': 5, 'floatstone': 5}, 'magic_seed'),
