...
from src import resources, visual, physics, saves_chooser, modloader, underia, mods
...
def update():
    ...
    for entity in game.entities:
        ...
        if d > 8000 + entity.IS_MENACE * 8000 or (d > 1200 + entity.IS_MENACE * 1200 and not entity.is_suitable(game.get_biome())):
    ...
    if game.get_biome() not in ['inner']:
        ...
        if game.stage > 3:
            underia.entity_spawn(underia.Entities.GhostFace, target_number=15, to_player_max=5000, to_player_min=2000, rate=0.3)
            underia.entity_spawn(underia.Entities.SadFace, target_number=15, to_player_max=5000, to_player_min=2000, rate=0.3)
            underia.entity_spawn(underia.Entities.AngryFace, target_number=15, to_player_max=5000, to_player_min=2000, rate=0.3)
            underia.entity_spawn(underia.Entities.TimeTrap, target_number=15, to_player_max=5000, to_player_min=2000, rate=0.4)
            underia.entity_spawn(underia.Entities.TimeFlower, target_number=15, to_player_max=5000, to_player_min=2000, rate=0.4)
